...
#
    if 'attrs' in kwargs:
        ...
        ax.errorbar( xdata, ydata,yerr = yerr, xerr = xerr, 
                    fmt=kwargs['fmt'] if 'fmt' in kwargs else '-o',label=kwargs['label'] if 'label' in kwargs else '')       
    #--- plot
    #
...
def GetAvalKunMethod( broken_bonds,  tcrltn ):
    times = broken_bonds.keys() 
    times.sort()
    times = np.array(times)

    dt = times[1:]-times[:-1] #--- time difference
    assert np.all(dt>0.0)

    trueFalse = dt < int(tcrltn) #--- within correlation time?
    ...
    try:
        ...
        ptr2 = 0
        while True:
            while not trueFalse[ ptr1 ]:
                ptr1 += 1
            assert trueFalse[ ptr1 ] #--- aval starts
            ptr2 = ptr1
            while trueFalse[ ptr2 ]:
                ptr2 += 1
            assert not trueFalse[ ptr2 ] #--- aval. ends
            #--- size
            xsum = 0
            for i in xrange(ptr1, ptr2):
                timei = times[ i ]
                xsum += len( broken_bonds[ timei ] )
            init_time += [times[ptr1]] 
            end_time += [times[ptr2]] 
            Size += [xsum]
            #--- assign ptr2 to ptr1
            ptr1 = ptr2
        ...
        pass
    df_avl = pd.DataFrame({'ti':init_time,'tf':end_time,'size':Size})
    return df_avl
